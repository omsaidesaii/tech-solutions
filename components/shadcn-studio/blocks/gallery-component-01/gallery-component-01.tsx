          ...
          {sections.map((section, sectionIndex) => (
            <div key={sectionIndex} className={cn({ 'grid grid-cols-2 gap-6': section.type === 'grid' })}>
              {section.images.map((image, imageIndex) => (
                <img key={imageIndex} src={image.src} alt={image.alt} className='h-full w-full rounded-lg object-cover grayscale hover:grayscale-0 transition-all duration-300' />
              ))}
            </div>
          ))}
        </div>
